...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		// Depth puts first, breadth puts last

		while(!p.isGoalState(frontier.peekAtFront().getState())) { // loop unless goal is reched by algorithm
			SearchNode front = frontier.removeFirst();
			explored.add(front);
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(front.getState());
			for (GridPos neighbour : childStates) {
				SearchNode n = new SearchNode(neighbour, front);
				if (!explored.contains(n)){
					if (this.insertFront) {
						frontier.addNodeToFront(new SearchNode(neighbour, front));
					}
					else {
						frontier.addNodeToBack(new SearchNode(neighbour, front));
					}
				}
			}
		}
